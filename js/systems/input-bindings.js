...
].map(action=>Object.freeze({...action,required:true,remappable:true})));
...
export function cloneInputBindings(profile=DEFAULT_INPUT_PROFILE){const source=typeof profile==="string"?INPUT_PROFILES[profile]:profile;if(!source)throw new TypeError(`Profil de commandes inconnu : ${profile}`);return Object.fromEntries(Object.entries(source).map(([action,codes])=>[action,[...codes]]));}
...
export function validateInputBindings(bindings,{requireAll=true}={}){const errors=[];if(!bindings||typeof bindings!=="object"||Array.isArray(bindings))errors.push("La configuration doit être un objet.");else{for(const action of INPUT_CATALOG){const codes=bindings[action.id];if(requireAll&&(!Array.isArray(codes)||!codes.length))errors.push(`Action obligatoire absente : ${action.id}`);}for(const[action,codes]of Object.entries(bindings)){if(!actionIds.has(action)){errors.push(`Action inconnue : ${action}`);continue;}if(!Array.isArray(codes)||!codes.length){errors.push(`Aucune touche pour : ${action}`);continue;}for(const code of codes){if(typeof code!=="string"||!code)errors.push(`Touche invalide pour : ${action}`);else if(reservedCodes.has(code))errors.push(`Touche réservée : ${code}`);}}}const conflicts=findInputConflicts(bindings);if(conflicts.length&&!INPUT_CONFLICT_POLICY.allowSharedCodes)errors.push(...conflicts.map(conflict=>`Conflit ${conflict.code} : ${conflict.actions.join(", ")}`));return Object.freeze({valid:errors.length===0,errors:Object.freeze(errors),conflicts});}

const readableCodes=Object.freeze({ArrowUp:"↑",ArrowDown:"↓",ArrowLeft:"←",ArrowRight:"→",ControlLeft:"CTRL",ControlRight:"CTRL",ShiftLeft:"MAJ",ShiftRight:"MAJ",Space:"ESPACE",Tab:"TAB",Enter:"ENTRÉE",MouseBack:"SOURIS 4",MouseForward:"SOURIS 5"});
...
export function formatInputCode(code){if(readableCodes[code])return readableCodes[code];if(/^Key[A-Z]$/.test(code))return code.slice(3);if(/^Digit\d$/.test(code))return code.slice(5);return String(code??"").toUpperCase();}
