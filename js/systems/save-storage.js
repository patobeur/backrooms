import {SAVE_SCHEMA_VERSION,SAVE_STORAGE_KEY,migrateExplorationSave,parseExplorationSave,serializeExplorationSave} from "./save-schema.js";
...
export function createExplorationSaveStorage({storage}={}){
  ...
  return Object.freeze({
    read(){if(!target)return{ok:false,status:"unavailable",save:null};let raw;try{raw=target.getItem(SAVE_STORAGE_KEY);if(!raw)return{ok:true,status:"empty",save:null};let parsed;try{parsed=JSON.parse(raw);}catch(error){return{ok:false,status:"invalid",save:null,error};}if(parsed?.version===SAVE_SCHEMA_VERSION)return{ok:true,status:"loaded",save:parseExplorationSave(raw)};let migrated;try{migrated=migrateExplorationSave(parsed);}catch(error){return{ok:false,status:"incompatible",save:null,error,version:parsed?.version};}try{target.setItem(SAVE_BACKUP_KEY,raw);target.setItem(SAVE_STORAGE_KEY,serializeExplorationSave(migrated));}catch(error){return{ok:false,status:"migration-write-failed",save:null,error};}return{ok:true,status:"migrated",save:migrated,fromVersion:parsed.version,backupKey:SAVE_BACKUP_KEY};}catch(error){return{ok:false,status:"invalid",save:null,error};}},
    ...
    remove(){if(!target)return{ok:false,status:"unavailable"};try{target.removeItem(SAVE_STORAGE_KEY);return{ok:true,status:"removed"};}catch(error){return{ok:false,status:"remove-failed",error};}},
