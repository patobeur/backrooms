...
export function createInputPreferences(profile=DEFAULT_INPUT_PROFILE){if(!INPUT_PROFILES[profile])throw new TypeError(`Profil inconnu : ${profile}`);return snapshot(profile,cloneInputBindings(profile),{source:"default"});}
...
export function remapInputBinding(preferences,action,code,{slot=0,replaceConflict=false,storage}={}){
  if(!actionIds.has(action))throw new TypeError(`Action inconnue : ${action}`);if(typeof code!=="string"||!code)throw new TypeError("Touche invalide.");if(reservedCodes.has(code))throw new TypeError(`Touche réservée : ${code}`);
