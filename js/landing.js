...
function renderExplorationReport(){
  ...
  fillReportSection("inventory",report.sections.inventory?reportList(report.inventory.map(item=>({primary:item.nameKey?translate(item.nameKey,item.parameters):item.type,doses:item.type.startsWith("water_")&&Number.isFinite(item.state.sips)?{value:Math.max(0,Math.min(4,item.state.sips)),max:4}:null,secondary:item.level?translate("report.inventory.origin",{level:item.level}):""}))):null);
